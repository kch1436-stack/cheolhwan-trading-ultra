...
with tabs[5]:
    ...
    with j2:
        j_tf = st.selectbox("시간봉", ["15M","1H","4H","1D"], index=1)
...
with tabs[7]:
    ...
    p_symbol = st.text_input("종목", value="BTC")
    p_tf = st.selectbox("시간봉", ["15M","1H","4H","1D"], index=1)
    p_dir = st.selectbox("방향", ["Long","Short"], key="post_dir")
    p_grade = st.selectbox("자리 등급", ["A급","B급","쓰레기"], index=0)
    p_entry = st.number_input("진입가", min_value=0.0, value=70000.0, step=10.0)
    p_stop = st.number_input("손절가", min_value=0.0, value=69300.0, step=10.0)
    p_tp1 = st.number_input("TP1", min_value=0.0, value=71200.0, step=10.0)
    p_tp2 = st.number_input("TP2", min_value=0.0, value=71800.0, step=10.0)
    p_notes = st.text_area("게시물 메모", value="구조 깨짐 후 눌림 확인, 추격은 금지.")
